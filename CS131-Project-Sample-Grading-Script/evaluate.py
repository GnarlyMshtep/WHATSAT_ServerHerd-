...
def evaluate_json(json_data, len_results=0):
    ...
    format_correct = len(
        set(["html_attributions", "results", "status"]) - set(json_data.keys())) == 0
...
def evaluate_info(feedback_string, expected_server, expected_client, expected_lat, expected_lng):
    ...
    correct_format = correct_length \
        ...
        and len(timestmp) == 1 \
        and len(location) == 2
    print(f'got server {feedback_elem[1]} but expected {expected_server}')
    correct_content = (correct_length, feedback_elem[1] == expected_server, feedback_elem[3] == expected_client, len(
        location) == 2, abs(float(location[0]) - float(expected_lat)) <= EPS, abs(float(location[1]) - float(expected_lng)) <= EPS)
...
def evaluate_flooding(results, target_results, max_item):
    ...
    for res in results:
        ...
        if same_json:
            same_json = evaluate_json(
                json_data_tmp, max_item) == json_correctness
