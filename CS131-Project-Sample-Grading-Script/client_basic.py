...
class SuperClient:
    ...
    async def start_server(self, server_name):
        command = 'nohup python{} {} {} &\n'.format(
            PYTHON_VER, self.server, server_name)
        os.system(command)
    ...
    def run_iamat(self, port, clientName, longitude, latitude):
        ...
        data = self.loop.run_until_complete(
            self.iamat(port, clientName, longitude, latitude))
        return data
    ...
    def run_whatsat(self, port, clientName, radius, maxItems):
        # start the loop
        data = self.loop.run_until_complete(
            self.whatsat(port, clientName, radius, maxItems))
        first_line = data.split('\n')[0]
        try:
            json_part = json.loads(data[len(first_line):]) if first_line.strip()[
                0] != "?" else dict()
    ...
    def test(self):
        ...
        data = self.safe_run_iamat(
            self.Juzang, "client", 34.068931, -118.445127)
        print(evaluate_info(
            data, self.port2server[self.Juzang], "client", 34.068931, -118.445127))
        first_line, json_part = self.safe_run_whatsat(
            self.Juzang, "client", 10, 5)
        # any other server can respond with correct info
        for test_target in [self.Bernard, self.Jaquez, self.Johnson, self.Clark]:
            first_line, json_part = self.safe_run_whatsat(
                test_target, "client", 10, 5)
            print(evaluate_info(
                first_line, self.port2server[self.Juzang], "client", 34.068931, -118.445127))
        ...
        first_line, json_part = self.safe_run_whatsat(
            self.Johnson, test_case["client"], test_case["radius"], test_case["max_item"])
        ...
        print("Test 3")
        self.start_all_servers()
        messages = ["IAMAT Clark +12-118", "IAMAT Clark +12-118 160000 123",
                    "AS Clark +0.25 testuser +12-118 1234"]
        total_tests = len(messages)
        total_correct = 0
        for message in messages:
            response_message = self.safe_run_errormsg(
                self.Clark, message).strip()
            print(evaluate_error(message, response_message))
        self.end_all_servers()

        # test 4 for flooding + sending weird arguments in WHATSAT
        print("===============")
        print("Test 4")
        self.start_all_servers()
        test_case = {
        ...
        # the record at Johnson should be its old copy, as it does not connect to Clark
        first_line, json_part = self.safe_run_whatsat(
            self.Johnson, test_case["client"], test_case["radius2"], test_case["max_item2"])
        print(evaluate_info(first_line, self.port2server[self.Johnson],
                            test_case["client"], test_case["latitude"], test_case["longitude"]))
        print(evaluate_json(json_part, test_case["max_item"]))
        self.end_all_servers()
...
if __name__ == '__main__':
    ...
    # this is in case we have other files to import from there
